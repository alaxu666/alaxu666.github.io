...
class PKRDataCrawler:
    ...
    def update_ebp_leader_info(self, df1):
        """使用FindEBPLeader函数更新EBP Leader信息"""
        try:
            ...
            # 调用FindEBPLeader函数获取EBP Leader信息
    ...
    def filter_data_by_date_range(self, df1):
        ...
        try:
            ...
            # 为每个项目创建多行（如果多个phase都在日期范围内）
            processed_rows = []
            for _, row in df_filtered.iterrows():
                project_has_valid_phases = False

                # 检查每个Phase的日期，如果在当前周或下周，则为该phase创建一行
                for col in date_columns:
                    if col in df1.columns and pd.notna(row[col]):
                ...
                # 如果项目没有任何phase在日期范围内，仍然保留一行（当前状态为空）
